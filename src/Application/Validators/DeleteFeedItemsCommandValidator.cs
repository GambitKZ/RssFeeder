...
public class DeleteFeedItemsCommandValidator : AbstractValidator<DeleteFeedItemsCommand>
{
    ...
    {
        _ = RuleFor(list => list.ListOfFeedId).NotNull()
            .Must(x => x.Count > 0).WithMessage("List with Id should not be Empty");

        _ = RuleForEach(list => list.ListOfFeedId)
            .Must(HaveValidGuid).WithMessage("List should contain valid GUIDs");
    }
